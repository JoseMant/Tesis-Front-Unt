...
@Component({
    selector       : 'carpetas-firma-secretaria-list',
    templateUrl    : './list.component.html',
    styles         : [
        /* language=SCSS */
        `
            .carpetas-firma-secretaria-grid {
                ...
                @screen lg {
                    grid-template-columns: 48px 112px 350px 250px 96px 250px 112px 150px;
                }
            }
            .fondo_snackbar {
...
export class CarpetasFirmasSecretariaGeneralListComponent implements OnInit, AfterViewInit, OnDestroy
{
    @ViewChild(MatPaginator) private _paginator: MatPaginator;
    @ViewChild(MatSort) private _sort: MatSort;
    @ViewChild('selectedResolucionNgForm') selectedResolucionNgForm: NgForm;
    @ViewChild('selectedDiplomasNgForm') selectedDiplomasNgForm: NgForm;
    ...
    selectedDiplomasForm: FormGroup;
   ...
   constructor(
    ...
    {   
        ...
        this.selectedDiplomasForm = this._formBuilder.group({
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    ...
                    return this._carpetasService.getCarpetasFirmasSecretaria(data, 0, this._paginator.pageSize, this._sort.active, this._sort.direction, query);
                }),
                map(() => {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    ...
                    if (this.searchInputControl.value) {
                        return this._carpetasService.getCarpetasFirmasSecretaria(data, this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value);
                    } else {
                        return this._carpetasService.getCarpetasFirmasSecretaria(data, this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction);
                    }
                }),
                map(() => {
                    this.isLoading = false;
    ...
    firmaSecretaria() {
        ...
        this._carpetasService.firmaSecretaria(data.idResolucion).subscribe((response) => {
            this.alert = {
                type   : 'success',
    ...
    descargarZip(): void
    {
        ...
        a.target = '_blanck'
        a.href = environment.baseUrl + 'download/diplomas/'+data.idResolucion
        a.click();
    ...
    selectDiplomas(event): void {
        const archivo = event.target.files[0];
        const data = this.selectedResolucionForm.getRawValue();
        this.selectedDiplomasForm.patchValue({archivo: archivo});
        this.selectedDiplomasForm.patchValue({idResolucion: data.idResolucion});
        this.uploadDiplomas();
    }
    ...
    uploadDiplomas(): void{
        ...
        formData.append('archivo', this.selectedDiplomasForm.getRawValue().archivo);
        formData.append('idResolucion', this.selectedDiplomasForm.getRawValue().idResolucion);
        // console.log(formData);
        // debugger;
        // Disable the form
        this.selectedDiplomasForm.disable();

        this._carpetasService.uploadDiplomas(formData)
        .pipe(
            finalize(() => {
                // Re-enable the form
                this.selectedDiplomasForm.enable();

                // Reset the form
                // this.selectedDiplomasNgForm.resetForm();
                this.asignando = false;
