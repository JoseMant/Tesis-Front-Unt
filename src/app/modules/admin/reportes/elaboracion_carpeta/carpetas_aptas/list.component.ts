...
export class ReporteCarpetasAptasTramitesListComponent implements OnInit, AfterViewInit, OnDestroy
{
        ...
        this.selectedReporteForm.get('idUnidad').valueChanges
            .pipe(
                switchMap(() => {
                    ...
                    return this._reportesService.getReporteCarpetasAptas(form.idUnidad, form.idDependencia, form.idPrograma, form.idTipo_tramite_unidad, form.cronograma)
                }),
                map(() => {
    ...
    verExcel(){
        ...
        link.setAttribute('href', environment.baseUrl + 'reporte/carpetas_aptas/excel/' + form.idDependencia+'/'+form.cronograma);
        ...
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    const form = this.selectedReporteForm.getRawValue();
                    return this._reportesService.getReporteCarpetasAptas(form.idUnidad, form.idDependencia, form.idPrograma, form.idTipo_tramite_unidad, form.cronograma, this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction)
                }),
                map(() => {
                    this.isLoading = false;
                })
