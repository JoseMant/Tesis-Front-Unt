    ...
    styles         : [
        ...
        `
            .reportes-elaboracion_carpeta-grid {
                ...
                @screen md {
                    grid-template-columns: 48px 112px 150px auto 110px 120px;
                }

                @screen lg {
                    grid-template-columns: 60px 150px 150px auto 250px 150px 220px;
                }
            }
            .fondo_snackbar {
...
export class ReporteCarpetasExpedientesListComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this.selectedReporteForm.get('idUnidad').valueChanges
            .pipe(
                switchMap(() => {
                    ...
                    return this._reportesService.getReporteExpedientes(form.idUnidad, form.idDependencia, form.idPrograma, form.idTipo_tramite_unidad, form.anio)
                }),
                map(() => {
        ...
        this.selectedReporteForm.get('anio').valueChanges
            .pipe(
                switchMap(() => {
                    this.isLoading = true;
                    const form = this.selectedReporteForm.getRawValue();
    ...
    changedUnidad(idUnidad: number) : void {
        ...
        this._reportesService.getAnio().subscribe((response)=>{
            this.anios = response;
        ...
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange, this._paginator.page).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    const form = this.selectedReporteForm.getRawValue();
                    return this._reportesService.getReporteExpedientes(form.idUnidad, form.idDependencia, form.idPrograma, form.idTipo_tramite_unidad, form.anio, this._paginator.pageIndex, this._paginator.pageSize, this._sort.active, this._sort.direction)
                }),
                map(() => {
                    this.isLoading = false;
                })
