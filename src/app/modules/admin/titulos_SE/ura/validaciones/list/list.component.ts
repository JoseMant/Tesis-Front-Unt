...
@Component({
    selector       : 'titulos-validados-list',
    templateUrl    : './list.component.html',
    styles         : [
        /* language=SCSS */
        `
            .titulos-validados-grid {
...
export class TitulosURAValidacionesListComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.isLoading = true;
                    return this._titulosService.getTitulosValidacionURA(0, 100, 'fecha', 'desc', query);
                }),
                map(() => {
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._titulosService.getTitulosValidacionURA(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._titulosService.getTitulosValidacionURA(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
    ...
    ngAfterViewInit(): void
    {
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._titulosService.getTitulosValidacionURA(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
