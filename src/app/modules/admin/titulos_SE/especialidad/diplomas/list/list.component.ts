...
@Component({
    selector       : 'titulos_SE-especialidad-diplomas-list',
    templateUrl    : './list.component.html',
    styles         : [
        /* language=SCSS */
        `
            .titulos_SE-especialidad-diplomas-grid {
...
export class TitulosEspecialidadDiplomasListComponent implements OnInit, AfterViewInit, OnDestroy
{
    @ViewChild(MatPaginator) private _paginator: MatPaginator;
    @ViewChild(MatSort) private _sort: MatSort;

    titulos_SE$: Observable<TituloInterface[]>;
    ...
    ngOnInit(): void
    {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.isLoading = true;
                    return this._titulosService.getTitulosDiplomasEspecialidad(0, 100, 'fecha', 'desc', query);
                }),
                map(() => {
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._titulosService.getTitulosDiplomasEspecialidad(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._titulosService.getTitulosDiplomasEspecialidad(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
    ...
    ngAfterViewInit(): void
    {
        if ( this._sort && this._paginator )
        {
            ...
            // Get titulos_SE if sort or page changes
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._titulosService.getTitulosDiplomasEspecialidad(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
