...
export class VouchersRechazadosListComponent implements OnInit, AfterViewInit, OnDestroy
{
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    ...
                    return this._vouchersService.getVouchersRechazados(0, 100, 'nro_tramite', 'asc', query);
                }),
                map(() => {
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._vouchersService.getVouchersRechazados(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._vouchersService.getVouchersRechazados(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
    ...
    ngAfterViewInit(): void
    {
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._vouchersService.getVouchersRechazados(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
