...
@Component({
    selector       : 'certificados-firma-decano-list',
    templateUrl    : './list.component.html',
    styles         : [
        /* language=SCSS */
        `
            .certificados-aprobados-grid {
...
export class CertificadosFirmaDecanoListComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.isLoading = true;
                    return this._certificadosService.getCertificadosFirmaDecano(0, 100, 'fecha', 'desc', query);
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._certificadosService.getCertificadosFirmaDecano(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._certificadosService.getCertificadosFirmaDecano(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
        ...
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    ...
                    return this._certificadosService.getCertificadosFirmaDecano(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
                }),
                map(() => {
                    this.isLoading = false;
                })
