...
export class TramiteListComponent implements OnInit, OnDestroy
{
    ...
    createTramite(): void{
        ...
        const requis = this.data.requisitos.find(element => element.responsable == this.user.idTipoUsuario && ((element.archivoPdf === undefined && element.extension === 'pdf') || (element.archivoImagen === undefined && element.extension === 'jpg'))
            && element.idRequisito !== 89 && element.idRequisito !== 90 && element.idRequisito !== 91 && element.idRequisito !== 102 && element.idRequisito !== 108 && element.idRequisito !== 114);
        if (requis) {
            this.alert = {
                type   : 'warn',
