...
export class GradosURADiplomasListComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._gradosService.getGradosDiplomasURA(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._gradosService.getGradosDiplomasURA(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
    ...
    ngAfterViewInit(): void
    {
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._gradosService.getGradosDiplomasURA(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
