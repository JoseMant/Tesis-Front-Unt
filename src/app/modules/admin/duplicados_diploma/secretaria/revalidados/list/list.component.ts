...
@Component({
    selector       : 'duplicados-revalidados-secretaria-list',
    templateUrl    : './list.component.html',
    styles         : [
        /* language=SCSS */
        `
            .duplicados-revalidados-secretaria-grid {
...
export class DuplicadosRevalidadosListComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this.searchInputControl.valueChanges
            .pipe(
                ...
                switchMap((query) => {
                    this.isLoading = true;
                    return this._duplicadosService.getDuplicadosRevalidados(0, 100, 'fecha', 'desc', query);
    ...
    cambioPagina(evento): void {
        if(this._sort.active) {
            this._duplicadosService.getDuplicadosRevalidados(evento.pageIndex, evento.pageSize, this._sort.active, this._sort.direction, this.searchInputControl.value).subscribe();
        }
        else {
            this._duplicadosService.getDuplicadosRevalidados(evento.pageIndex, evento.pageSize, 'nro_tramite', 'asc', this.searchInputControl.value).subscribe();
        }
    }
    ...
    ngAfterViewInit(): void
    {
        if ( this._sort && this._paginator )
        {
            ...
            merge(this._sort.sortChange).pipe(
                switchMap(() => {
                    this.isLoading = true;
                    return this._duplicadosService.getDuplicadosRevalidados(Number(this.pagination.page), Number(this.pagination.size), this._sort.active, this._sort.direction, this.searchInputControl.value);
